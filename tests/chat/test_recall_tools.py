...
def _add(store, fingerprint, *, repo="", message="boom", report="", **kwargs):
...
async def test_search_matches_the_error_text(store):
    _add(store, "fp1", repo="acme/api", message="KeyError: discount")
    _add(store, "fp2", repo="acme/api", message="TimeoutError on payments")

    result = await _tool(incident_tools(store), "search_incidents")(query="discount")
...
async def test_search_matches_inside_the_stored_report(store):
    ...
    _add(store, "fp1", repo="acme/api", report="root cause: cart/totals.py:88")

    result = await _tool(incident_tools(store), "search_incidents")(query="totals.py")
...
async def test_search_is_case_insensitive(store):
    ...
    result = await _tool(incident_tools(store), "search_incidents")(query="keyerror")
    assert json.loads(result)["matched"] == 1


async def test_an_empty_query_lists_recent_incidents(store):
    for n in range(3):
        _add(store, f"fp{n}", repo="acme/api")

    payload = json.loads(await _tool(incident_tools(store), "search_incidents")())
    assert payload["matched"] == 3


async def test_search_filters_by_repo(store):
    _add(store, "fp1", repo="acme/api")
    _add(store, "fp2", repo="acme/web")

    result = await _tool(incident_tools(store), "search_incidents")(repo="acme/web")
    payload = json.loads(result)
...
async def test_search_filters_by_artifact_kind(store):
    ...
    _add(store, "fp1", repo="acme/api", url="https://gh/pr/1", artifact=ARTIFACT_PR)
    _add(store, "fp2", repo="acme/api", url="https://gh/i/2", artifact=ARTIFACT_ISSUE)

    result = await _tool(incident_tools(store), "search_incidents")(artifact="pr")
    payload = json.loads(result)
...
async def test_search_filters_by_status(store):
    _add(store, "fp1", repo="acme/api")
    _add(store, "fp2", repo="acme/api", failures=1)

    result = await _tool(incident_tools(store), "search_incidents")(status="failed")
...
async def test_search_reports_the_total_when_limiting(store):
    ...
    payload = json.loads(
        await _tool(incident_tools(store), "search_incidents")(limit=2)
    )
...
async def test_no_match_names_the_repos_that_do_have_incidents(store):
    ...
    result = await _tool(incident_tools(store), "search_incidents")(query="nothing")
...
async def test_a_long_report_is_previewed_not_dumped(store):
    ...
    _add(store, "fp1", repo="acme/api", report="x" * 5000)

    payload = json.loads(
        await _tool(incident_tools(store), "search_incidents")(query="fp1")
    )
    assert len(payload["incidents"][0]["report"]) <= REPORT_PREVIEW + 1


async def test_a_failed_incident_shows_its_attempt_count(store):
    _add(store, "fp1", repo="acme/api", failures=MAX_ATTEMPTS)

    payload = json.loads(
        await _tool(incident_tools(store), "search_incidents")(status="failed")
    )
...
async def test_get_incident_returns_the_full_report(store):
    _add(store, "fp1", repo="acme/api", report="y" * 5000)

    result = await _tool(incident_tools(store), "get_incident")(
...
async def test_get_incident_with_the_wrong_repo_says_where_it_lives(store):
    ...
    result = await _tool(incident_tools(store), "get_incident")(fingerprint="fp1")
    assert "acme/api" in result
    assert "Pass the matching repo" in result


async def test_get_incident_unknown_fingerprint(store):
    result = await _tool(incident_tools(store), "get_incident")(fingerprint="nope")
...
async def test_stats_counts_by_status_and_repo(store):
    _add(store, "fp1", repo="acme/api", cost=0.01)
    _add(store, "fp2", repo="acme/web", cost=0.02)
    _add(store, "fp3", repo="acme/api", failures=MAX_ATTEMPTS)

    payload = json.loads(await _tool(incident_tools(store), "incident_stats")())
...
async def test_search_conversations_finds_an_earlier_session(memory):
    ...
    result = await _tool(recall_tools(memory, current), "search_conversations")(
        query="fix mode"
    )
...
async def test_recall_session_lists_sessions_when_given_no_id(memory):
    ...
    payload = json.loads(
        await _tool(recall_tools(memory, current), "recall_session")()
    )
...
async def test_recall_session_reads_back_the_messages(memory):
    ...
    payload = json.loads(
        await _tool(recall_tools(memory, current), "recall_session")(session_id=old)
    )
...
async def test_recall_session_unknown_id(memory):
    current = memory.start_session()
    result = await _tool(recall_tools(memory, current), "recall_session")(
        session_id=999
    )
